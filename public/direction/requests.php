...
$st = $pdo->query(
    "SELECT en.dossier_code, en.status, en.created_at, en.submitted_at,
            el.id AS elector_id, el.nom, el.prenom, el.cni, el.profile_photo
     FROM enrollments en
     JOIN electors el ON el.id = en.elector_id
     WHERE en.status IN ('SUBMITTED','PRE_ENROLLED')
     ORDER BY en.id DESC"
);
$rows = $st->fetchAll();
...
<div class="card card-soft">
  <div class="card-body p-4">
    <?php if (!$rows): ?>
      <div class="text-muted">Aucune demande en attente.</div>
    <?php else: ?>
          ...
          <tbody>
            <?php foreach ($rows as $r): ?>
              <tr>
                ...
                <td>
                  <?= e($r['nom'] . ' ' . $r['prenom']) ?>
                  <div class="text-muted small"><?= e($r['cni']) ?></div>
                </td>
                <td><span class="badge text-bg-secondary"><?= e($r['status']) ?></span></td>
                <td class="text-muted small"><?= e($r['submitted_at'] ?? '-') ?></td>
                <td class="text-end">
                  <a class="btn btn-sm btn-outline-dark" href="<?= e(base_url($config, 'direction/view.php?dossier=' . urlencode($r['dossier_code']))) ?>">Voir</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
